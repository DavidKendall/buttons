 ...
 *        This program uses a small, simple library of GPIO functions.
...
int main() {
  gpioPin_t pin[9];
  bool flashing[4] = {false, false, false, false};
	uint32_t i;
    ...
    for (i = LED1; i <= LED4; i++) {
		...
		if (flashing[LED1] || flashing[LED2] || flashing[LED3] || flashing[LED4]) {
		  delay(1000);
		}
	}
}
